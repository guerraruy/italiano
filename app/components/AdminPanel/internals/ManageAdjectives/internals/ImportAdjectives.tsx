...
export default function ImportAdjectives({ onError, onSuccess }: ImportAdjectivesProps) {
  const [importAdjectives, { isLoading: importingAdjectives }] = useImportAdjectivesMutation()

  const [adjectiveJsonContent, setAdjectiveJsonContent] = useState<string>('')
  const [adjectiveConflicts, setAdjectiveConflicts] = useState<ConflictAdjective[]>([])
  const [adjectiveConflictResolutions, setAdjectiveConflictResolutions] = useState<{
    [italian: string]: 'keep' | 'replace'
  }>({})
  const [showAdjectiveConflictDialog, setShowAdjectiveConflictDialog] = useState(false)
  const [showFormatInfoDialog, setShowFormatInfoDialog] = useState(false)

  const handleAdjectiveFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
          ...
          {adjectiveJsonContent && (
            <Box sx={{ mt: 2 }}>
              <Typography variant='body2' color='text.secondary' gutterBottom>
                Preview: {Object.keys(JSON.parse(adjectiveJsonContent)).length} adjectives
                ready to import
              </Typography>
              <Button
                variant='contained'
      ...
      >
        ...
        <DialogContent>
          <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
            The following adjectives already exist in the database. Choose whether to
            ...
            <Paper key={conflict.italian} sx={{ p: 2, mb: 2 }}>
              ...
              <Stack direction='row' spacing={2}>
                ...
                <Button
                  ...
                  color='secondary'
                  onClick={() =>
                    handleAdjectiveConflictResolution(conflict.italian, 'replace')
                  }
                >
                  Replace with New
                </Button>
